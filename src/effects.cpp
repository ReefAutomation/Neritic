...
void effect_lightning() {
  ...
  if (colors.size() > 0) {
    uint32_t c = (uint32_t)strtoul(
        colors[0].c_str() + (colors[0][0] == '#' ? 1 : 0), nullptr, 16);
    unpack_rgbw(c, baseR, baseG, baseB, baseW);
  }
  if (colors.size() > 1) {
    uint32_t c =
        (uint32_t)strtoul(colors[colors.size() - 1].c_str() +
                              (colors[colors.size() - 1][0] == '#' ? 1 : 0),
                          nullptr, 16);
  ...
  if (!inBurst && now - lastFlash > nextDelay) {
    ...
    uint8_t intensity =
        state.params.intensity > 0 ? state.params.intensity : 255;
    float minFlash =
        0.1f + 0.7f * (intensity / 255.0f); // min intensity 0.1-0.8
    float maxFlash =
  ...
  if (inBurst) {
    ...
    if (burstFlashIdx == burstFlashCount - 1 &&
        (now - flashTime > flashDuration / 2)) {
      float t = 1.0f - float(now - flashTime) / float(flashDuration);
      if (t < 0.2f)
        flashIntensity *= t / 0.2f;
    }
  } else {
    flashIntensity = 0.0f;
  }
  ...
  for (size_t i = 0; i < g_ledCount; ++i) {
    ...
    if (inBurst && flashIntensity > 0.0f) {
      ...
      // Use a hash of flashStart, flashLen, and i for deterministic randomness
      // per flash
      uint32_t hash = (uint32_t)(flashStart ^ (i * 2654435761UL) ^
                                 (flashLen * 374761393UL));
