...
# SCons import inside main() to avoid errors when run outside PlatformIO
def get_env():
    """Get the SCons environment when running via PlatformIO."""
    try:
...
def to_inc(infile, outfile):
    ...
    outfile - path to output .inc file
...
def main(env=None):
    """
    Main entry point: processes all assets, and logs results.
    Exits with error if any asset fails to embed.
    Assets should be built first via 'npm run build' or as part of extra_scripts.
    
    env - Optional SCons Environment object (passed by PlatformIO)
         If None, skips IsCleanTarget() check and runs anyway.
    """
    # Only skip if we actually have an env object passed
    env_obj = get_env()
    if env_obj is not None and env_obj.IsCleanTarget():
        return
    ...
    for asset in ASSETS:
        ...
        ok = process_asset(src_path, inc_path, force=False)  # Deletion handled by clean_inc_folder.py if needed
